...
export default function SupermarketGrid({ prices, setPrices, weather }: SupermarketGridProps) {
  ...
  const triggerScraper = async () => {
    ...
    setScrapeStep('Crawling Keells Super (keellssuper.com)...');

    try {
      setScrapeStep('Crawling Cargills Online (cargillsonline.com)...');
      const items = prices.map((p) => p.itemName);
      ...
      setScrapeStep('Merging live supermarket prices into catalog...');

      if (data.success && data.prices?.length) {
        setPrices(data.prices);
        setScrapedSuccessfully(true);
        setScrapeStep(`Done — ${data.liveCount} item(s) scraped from store sites.`);
      } else {
        setScrapeStep('Crawl returned no live prices — check Firecrawl/Scrape.do keys in .env');
      }
    } catch {
      setScrapeStep('Scrape failed — using catalog fallback.');
    } finally {
      setScraping(false);
      setTimeout(() => {
        setScrapeStep('');
    ...
    <div className="space-y-8 animate-fade-in text-[#2D332D]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h2 className="text-3xl font-serif italic font-bold text-[#14532D] mt-1">{AGENT_DISPLAY_NAMES.priceCatalogGrid}</h2>
          <p className="text-[#15803D] mt-2 text-sm leading-relaxed max-w-3xl">
            Agent 7 crawls Keells & Cargills live; pola/wholesale via market search (AnyPrice + SerpAPI). Prices cache in Supabase after successful crawls.
          </p>
        </div>
      ...
      {scrapedSuccessfully && (
        <div className="bg-[#F0FDF4] border border-[#BBF7D0] p-4 rounded-xl text-[#14532D] text-xs flex items-center gap-2 animate-bounce">
          ...
          <span>Live supermarket prices synced. Check Data sources for Keells/Cargills links.</span>
        </div>
      )}
